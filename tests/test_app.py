...
def test_add_ip_to_targets_fails_closed_without_remote_user_id(app_module):
    ...
    app = app_module
    results = app.add_ip_to_targets("1.2.3.4", remote_user="", remote_user_id="")
    assert results["pangolin"]["ok"] is False
    assert "Remote-User" in results["pangolin"]["detail"]
    assert results["crowdsec"]["detail"] == "not reached"
...
def test_add_ip_to_targets_intersection_filters_by_role(monkeypatch, app_module):
    ...
    results = app.add_ip_to_targets("1.2.3.4", remote_user="denise@example.com", remote_user_id="user-denise")
...
def test_add_ip_to_targets_fails_closed_empty_intersection(monkeypatch, app_module):
    ...
    results = app.add_ip_to_targets("1.2.3.4", remote_user="nobody@example.com", remote_user_id="user-nobody")
...
def test_add_ip_to_targets_fails_closed_user_not_found(monkeypatch, app_module):
    ...
    results = app.add_ip_to_targets("1.2.3.4", remote_user="ghost@example.com", remote_user_id="user-ghost")
...
def test_add_ip_to_targets_fails_closed_on_roles_api_error(monkeypatch, app_module):
    ...
    results = app.add_ip_to_targets("1.2.3.4", remote_user="joe@example.com", remote_user_id="user-joe")
...
def test_add_ip_to_targets_authorised_via_direct_user_assignment(
    ...
    results = app.add_ip_to_targets("1.2.3.4", remote_user="direct@example.com", remote_user_id="user-direct")
...
def test_rate_limit_skips_api_on_repeat_call(monkeypatch, app_module):
    ...
    first = app.add_ip_to_targets("1.2.3.4", remote_user="joe@example.com", remote_user_id="user-abc")
    assert first["pangolin"]["ok"] is True
    calls_after_first = api_calls["count"]
    assert calls_after_first > 0

    second = app.add_ip_to_targets("1.2.3.4", remote_user="joe@example.com", remote_user_id="user-abc")
    assert second["pangolin"]["ok"] is True
    assert api_calls["count"] == calls_after_first, (
        "API should not be called again within the rate limit window"
    )
...
def test_rate_limit_zero_disables_limiting(monkeypatch, app_module):
    ...
    app.add_ip_to_targets("1.2.3.4", remote_user="joe@example.com", remote_user_id="user-abc")
    calls_after_first = api_calls["count"]
    assert calls_after_first > 0
...
def _make_handler_for_ip_test(headers: dict, socket_ip: str = "10.20.30.40"):
    ...
    ctx = {
        ...
        "now_utc_iso": lambda: "2025-01-01T00:00:00+00:00",
        "save_state": lambda: None,
        "add_ip_to_targets": lambda ip, remote_user="", remote_user_id="": {},
        "banner_png": b"",
        "banner_gif": b"",
        "redact_headers_for_log": lambda h: h,
    }
...
def test_rate_limit_caching_success_only(app_module, monkeypatch):
    ...
    def fake_fail(ctx, org_id, username, remote_user_id):
        calls.append(username)
        raise RuntimeError("api down")

    monkeypatch.setattr(app, "pg_filter_resources_for_user", fake_fail)

    res1 = app.add_ip_to_targets("1.1.1.1", remote_user="user-1", remote_user_id="user-1")
    assert res1["pangolin"]["ok"] is False
    assert len(calls) == 1

    # 2) Second call should NOT be rate limited and should call filter_resources_for_user again
    res2 = app.add_ip_to_targets("1.1.1.1", remote_user="user-1", remote_user_id="user-1")
    assert res2["pangolin"]["ok"] is False
    assert len(calls) == 2
...
def test_rate_limit_cache_keyed_by_ip_and_user(app_module, monkeypatch):
    ...
    def fake_success(ctx, org_id, username, remote_user_id):
        calls.append(username)
        return [{"resourceId": 5, "name": "Res", "fullDomain": "d.com", "ssl": True}]
    ...
    # 2) Check in second user from same IP - should NOT hit cache because of different remote_user
    res2 = app.add_ip_to_targets("1.1.1.1", remote_user="user-2", remote_user_id="user-2")
    assert res2["pangolin"]["ok"] is True
    assert len(calls) == 2

    # 3) Check in first user again - should be rate limited and return cached result
    res3 = app.add_ip_to_targets("1.1.1.1", remote_user="user-1", remote_user_id="user-1")
    assert res3["pangolin"]["ok"] is True
    assert len(calls) == 2  # No new call
...
def test_dashboard_redirection_and_button(temp_state_file, monkeypatch):
    ...
    def fake_filter(ctx, org_id, username, remote_user_id):
        if state_mock["succeed"]:
            return [
                {
                    "resourceId": 5,
