...
def add_ip_to_targets(ip: str, remote_user: str = "", remote_user_id: str = "") -> dict:
    ...
    Requires remote_user and remote_user_id (forwarded by Pangolin/Badger 1.4.1+).
    ...
    if not remote_user or not remote_user_id:
        return _fail(
            "Remote-User or Remote-User-Id header not present — cannot identify user. "
            "Ensure this resource uses SSO authentication in Pangolin and Badger is 1.4.1+."
        )
